...
class RealtimeCall {
     ...
     */
    async endCall() {
        console.log('[RealtimeCall] Ending call and releasing resources...');

        // Stop all media stream tracks (this releases the microphone)
        if (this.mediaStream) {
            this.mediaStream.getTracks().forEach(track => {
                console.log('[RealtimeCall] Stopping track:', track.kind);
                track.stop();
            });
            this.mediaStream = null;
        }
        ...
        this.isCallActive = false;
        console.log('[RealtimeCall] ✅ Call ended and microphone released');
    }
 ...
 * @param {HTMLButtonElement} buttonElement - Optional button element to update state during call lifecycle
 */
export async function makeCall(instructions, voice = 'alloy', additionalOptions = {}, buttonElement = null) {
    console.log('[makeCall] Called with instructions:', instructions);
    console.log('[makeCall] Voice:', voice);
    if (buttonElement) {
        console.log('[makeCall] Button element provided for state tracking');
    }
